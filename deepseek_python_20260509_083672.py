# app.py - Complete Recharge Website with Money Request History
import os
import uuid
import random
import re
...
# Pagination Class
class Pagination:
    """Simple pagination class for transaction history"""
...
def update_wallet(user_id, amount, transaction_type, description, reference_id=None):
    """Deduct or add money from wallet"""
    try:
...
# Webhook endpoint to update recharge status
@app.route('/update-recharge-status', methods=['POST'])
def update_recharge_status():
    """API endpoint to update recharge status - Deducts money when status becomes SUCCESS"""
    try:
        data = request.get_json()
        txn_id = data.get('txn_id')
        new_status = data.get('status')
        
        if not txn_id or new_status not in ['SUCCESS', 'FAILED']:
            return jsonify({'error': 'Invalid request'}), 400
        
        response = supabase.table('transactions').select('*').eq('txn_id', txn_id).execute()
        ...
        if new_status == 'SUCCESS':
            user_response = supabase.table('user_profiles').select('wallet_balance').eq('id', transaction['user_id']).execute()
            if user_response.data and user_response.data[0]['wallet_balance'] >= transaction['amount']:
                if update_wallet(transaction['user_id'], transaction['amount'], 'DEBIT', 
                                f"{transaction['type']} recharge to {transaction['number']} ({transaction['operator']}) - SUCCESS", 
                                txn_id):
                    supabase.table('transactions').update({'status': 'SUCCESS'}).eq('txn_id', txn_id).execute()
                    return jsonify({'message': 'Recharge successful, money deducted'}), 200
                else:
                    return jsonify({'error': 'Failed to deduct money'}), 500
            else:
                supabase.table('transactions').update({'status': 'FAILED'}).eq('txn_id', txn_id).execute()
                return jsonify({'error': 'Insufficient balance, transaction marked as failed'}), 400
        
        elif new_status == 'FAILED':
            supabase.table('transactions').update({'status': 'FAILED'}).eq('txn_id', txn_id).execute()
            return jsonify({'message': 'Transaction marked as failed, no money deducted'}), 200
        
    except Exception as e:
        print(f"Error updating recharge status: {e}")
        ...
        try:
            ...
            if response.data:
                ...
                if checkpw(password.encode('utf-8'), user['password'].encode('utf-8')):
                    ...
                    session['user_id'] = str(user['id'])  # Convert to string to ensure consistent type
...
def wallet():
    ...
    try:
        ...
        money_requests = supabase.table('money_requests')\
            .select('*')\
            .eq('user_id', session['user_id'])\
            .order('created_at', desc=True)\
            .execute()
        money_requests = money_requests.data if money_requests else []
...
def do_recharge():
    ...
    try:
        ...
        wallet_txn = {
            ...
            'amount': amount,
            'type': 'PENDING',
            'description': f'{recharge_type} recharge to {mobile} ({operator}) - Waiting for confirmation',
            'reference_id': txn_id,
            'date': datetime.utcnow().isoformat()
        }
        supabase.table('wallet_transactions').insert(wallet_txn).execute()
        
        flash(f'Recharge request submitted! Transaction ID: {txn_id}. Status: PENDING. Money will be deducted when status changes to SUCCESS.', 'info')
...
def do_dth_recharge():
    ...
    try:
        ...
        wallet_txn = {
            ...
            'amount': amount,
            'type': 'PENDING',
            'description': f'DTH recharge to {subscriber_id} ({operator}) - Waiting for confirmation',
            'reference_id': txn_id,
            'date': datetime.utcnow().isoformat()
        }
        supabase.table('wallet_transactions').insert(wallet_txn).execute()
        
        flash(f'DTH recharge request submitted! Transaction ID: {txn_id}. Status: PENDING. Money will be deducted when status changes to SUCCESS.', 'info')
...
def history():
    ...
    try:
        ...
        all_transactions = query.execute()
        
        # Calculate total and pagination
        total = len(all_transactions.data) if all_transactions.data else 0
        
        # Get paginated items
        start = (page - 1) * per_page
        end = start + per_page
        paginated_items = all_transactions.data[start:end] if all_transactions.data else []
        
        # Create pagination object
        paginated_transactions = Pagination(paginated_items, page, per_page, total)
