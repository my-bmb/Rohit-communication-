...
def update_wallet(user_id, amount, transaction_type, description, reference_id=None):
    try:
        user_response = supabase.table('users').select('wallet_balance').eq('id', user_id).execute()
...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            response = supabase.table('users').select('*').eq('mobile', mobile).eq('is_active', True).execute()
...
def wallet():
    ...
    return render_template('wallet.html', user=user, transactions=wallet_transactions)
...
def add_money():
    ...
    if amount > 10000:
        flash('Maximum amount per transaction is ₹10,000', 'danger')
        return redirect(url_for('wallet'))
    ...
    if payment_success:
        ...
        if update_wallet(session['user_id'], amount, 'CREDIT', 
                        f'Wallet add via dummy payment', payment_id):
            flash(f'Successfully added ₹{amount:.2f} to wallet', 'success')
        else:
            flash('Failed to add money. Please try again.', 'danger')
    else:
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form method="POST" action="{{ url_for('add_money') }}">
            ...
            <small class="text-muted">Min: ₹1 | Max: ₹10,000 per transaction</small>
        </form>
    </div>
</div>
